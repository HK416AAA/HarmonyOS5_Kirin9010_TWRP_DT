# HarmonyOS compatibility overlay

TWRP ships an Android-style userspace and expects Android properties
(`prop.default`, `ro.*`, read by its init). HarmonyOS has neither: it stores
device parameters as `.para` files that the param service reads and init
consumes through `setparam` / `param:<key>=<value>` triggers.

This directory bridges the two. Everything about device identity lives in
HarmonyOS format, and the Android side is generated from it.

## Prop handling

| File | Format | Role |
|---|---|---|
| `param/ohos.para` | HarmonyOS `.para` | **Source of truth** for device identity |
| `param/ohos.para.dac` | HarmonyOS DAC | ownership/permissions for those params |
| `param/ohos.startup.para` | HarmonyOS `.para` | `persist.sys.usb.config=hdc`, boot events |
| `param/hilog.para` + `.dac` | HarmonyOS `.para` | logging params for the bundled `libhilog.so` |
| `param/hdc.para` + `.dac` | HarmonyOS `.para` | HDC transport (USB enabled) |
| `prop.default` | Android props | **Generated** from `ohos.para`; reference only, not installed |
| `prop-overrides.mk` | make fragment | **Generated**; `device.mk` includes it (`PRODUCT_PROPERTY_OVERRIDES` + `PRODUCT_PRODUCT_PROPERTIES`) |
| `init.kirin9010.cfg` | HarmonyOS init | **Source of truth**: device bring-up jobs |
| `ohos.recovery.cfg` | HarmonyOS init | **Source of truth**: recovery-mode jobs |
| `generated/init.recovery.harmony.rc` | Android init | **Generated** from both `.cfg` files by `scripts/cfg2rc.py` |
| `ueventd.config` | HarmonyOS ueventd | device node table, installed at its stock path |

`param/ohos.para` uses the same namespaces as the stock image
(`const.product.`, `const.build.`, `ohos.boot.`), and the `.dac` file mirrors
the stock `etc/param/ohos.para.dac` syntax:

```
<name-or-prefix>=<owner>:<group>:<mode>[:type]      # prefix ends with a dot
```

Both files are installed at `recovery/root/system/etc/param/` (i.e. `/etc/param/`
through the ramdisk's `etc -> /system/etc` symlink), the same path the stock
OpenHarmony updater uses.

## Why the props are generated

Keeping two hand-written prop files in sync is how device trees rot. Instead:

```bash
scripts/para2prop.py harmony/param/ohos.para \
    -o harmony/prop.default \
    --mk harmony/prop-overrides.mk
```

`scripts/apply-harmony-adaptation.sh` runs this automatically before every
build, so editing `ohos.para` is enough to change both sides.

`prop.default` is only a human-readable listing. The properties that actually
reach the image come from `prop-overrides.mk`: the build system generates
`recovery/root/prop.default` itself (by concatenating the partition
`build.prop` files), so shipping our own file at that path is a duplicate-rule
error. Device identity and TWRP runtime values are fed through
`PRODUCT_PROPERTY_OVERRIDES` (`system/build.prop`). The security/debug values
(`ro.secure`, `ro.debuggable`, ...) are fed through
`PRODUCT_PRODUCT_PROPERTIES` instead, because `core/main.mk` already assigns
them to `ADDITIONAL_SYSTEM_PROPERTIES` and a second system assignment makes
`post_process_props` fail with "found duplicate sysprop assignments". Both
`system/build.prop` and `product/etc/build.prop` are concatenated into the
generated `recovery/root/prop.default`, product last, so recovery still ends up
with these values. Values that contain whitespace (the product name, the
software version) cannot be expressed as a make list word and appear only in
`prop.default`.

The translation is not mechanical:

- `const.product.model` becomes `ro.product.model`, `ohos.boot.ufs` becomes
  `ro.boot.ufs`, and so on (`MAPPING` in `scripts/para2prop.py`).
- `const.secure=1` / `const.debuggable=0` describe the *secure HarmonyOS
  system*, so they are preserved as `ro.ohos.secure` / `ro.ohos.debuggable` and
  are **not** copied onto `ro.secure` / `ro.debuggable` — recovery needs those
  opposite (`0` / `1`) to allow root adb.
- Recovery-runtime values with no HarmonyOS equivalent (device codename
  `kirin9010`, USB ids, LCD density) are a static block at the end.

## Init handling

TWRP's init is Android init: it reads `.rc`, not the HarmonyOS JSON `.cfg`.
Rather than keep a hand-written `.rc` next to the `.cfg` — which is how the two
drift, and which is why one of them silently never ran — the `.cfg` files stay
authoritative and the `.rc` TWRP loads is generated:

```bash
scripts/cfg2rc.py harmony/init.kirin9010.cfg harmony/ohos.recovery.cfg \
    --out harmony/generated/init.recovery.harmony.rc
```

Bindings that matter:

- job name -> trigger: `pre-init`->`early-init`, `init`->`init`,
  `post-fs`->`fs`, `late-fs`->`late-fs`, `post-fs-data`->`post-fs-data`,
  `boot`->`boot`;
- `setparam K V` -> `setprop K V`;
- `mount <type> <source> <target>` -> `mount <source> <target> <type>`
  (HarmonyOS puts the filesystem type first, Android last);
- `mkdir`, `chown`, `chmod`, `symlink`, `write`, `wait`, `rmdir`,
  `restorecon`, `export` pass through unchanged;
- commands with no Android equivalent (`mkswap`, `swapon`, `swapoff`,
  `seteswap`, `start`, `stop`) are dropped with a warning, as are `param:`
  conditions that need the HarmonyOS param service (`&&`, `||`, `${...}`,
  wildcards).

`init.recovery.kirin9010.rc` is only an entry point: it imports the generated
file and the HDC file. The generated file is committed so the tree builds as-is,
and regenerated by `scripts/apply-harmony-adaptation.sh` on every build.

Since recovery does not start the HarmonyOS param service, the `.para` files
are inert at runtime under TWRP. They exist so the recovery environment stays
consistent with the system it repairs, and so the device identity has one
authoritative, HarmonyOS-native definition.

## ueventd handling

`ueventd.config` is the authoritative HarmonyOS device-node table, installed at
its stock path (`/system/etc/ueventd.config`). Android's ueventd cannot consume
it, and no Android rc is generated from it:

- ueventd resolves owners with `getpwnam()`/`getgrnam()`, which take names, not
  numbers. The stock table uses numeric HarmonyOS gids (`input=6696`,
  `camera_host=3028`, ...), so every line would be rejected as "invalid gid";
- ueventd's extra-config path `/ueventd.<ro.hardware>.rc` is only parsed when
  `ro.product.first_api_level` is old enough, and this tree never sets
  `PRODUCT_SHIPPING_API_LEVEL`, so such a file would never be read;
- `/system/etc/init/` is not scanned by ueventd at all, so the previous
  `ueventd.harmony.rc` there never ran.

The practical consequence is small: TWRP runs as root and ueventd still creates
every node with default ownership, so the nodes stay reachable. The table is
kept as the HarmonyOS-native record.

## What was ported from the updater image

The stock OpenHarmony 5.0.5.165 `updater_ramdisk.img` is the runtime reference.
The following were lifted from its `etc/` tree:

- **Device nodes** — `etc/ueventd.config` is carried through verbatim as
  `ueventd.config`; see "ueventd handling" above for why it is not converted to
  an Android rc.
- **Parameters** — `ohos.startup.para` (`persist.sys.usb.config=hdc`) and
  `hilog.para`/`.dac` join the existing `ohos.para` and `hdc.para` pairs.
- **Partition paths** — the stock `etc/fstab.updater` mounts from
  `/dev/block/by-name/*`, while the device fstab uses
  `/dev/block/platform/fa500000.ufs/by-name/*`. `init.kirin9010.cfg` links the
  short path to the UFS path so both resolve.
- **USB / FunctionFS** — `etc/init.usb.cfg` and `etc/init.usb.configfs.cfg`
  are the basis of `hdc/init.recovery.hdc.rc` and `hdc/hdc-usb.sh`. The configfs
  gadget and its controller (`efc00000.dwc3`) are declared in
  `init.kirin9010.cfg`, which also selects TWRP's configfs path through
  `sys.usb.configfs`.
